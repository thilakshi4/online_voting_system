<?php
$title = 'Election Commision Officer Requests';
...
$res = mysqli_query($conn, "SELECT * FROM req_reg_officer WHERE ref_number='$ref'");
...
<nav class="navbar navbar-expand-sm navbar navbar-dark bg-primary">
    <div ><a class="navbar-brand" href="request_ec.php.">&nbsp;&nbsp;Back</a></div>
...
<div align="center" class="style_form ">
    ...
    function insertData() {
        ...
        $sql = "INSERT INTO staff_login (staff_id, username, password,user_level,name_with_initials,NIC,mobile_no,status)
            SELECT ref_number, NIC, password,user_level,name_with_initials,
            NIC,mobile_no,'Successfully Registered'
            FROM req_reg_officer
            WHERE ref_number='$ref'";
        $result = mysqli_query($GLOBALS['conn'], $sql);


        //delete relavent result from req_reg_officer_gn table
        $sql_del = "DELETE FROM req_reg_officer WHERE ref_number='$ref'";
    ...
    function rejectData() {
        ...
        $sql_mob = "select mobile_no FROM req_reg_officer WHERE `ref_number`='$ref' ";
        ...
        $sql = "INSERT INTO rejected_officer_registration (user_level,ref_number,name_with_initials,NIC,mobile_no,password,commnent)
            SELECT user_level,ref_number,name_with_initials,NIC,mobile_no,password,'$comments'
            FROM req_reg_officer
            WHERE ref_number='$ref'";
        $result = mysqli_query($GLOBALS['conn'], $sql);
