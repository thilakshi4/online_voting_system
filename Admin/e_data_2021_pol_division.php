...
<div class="container text-center home_box col-sm-10">
    <div align="center">
        <p><b><font size=5 color='blue'> Polling division vice vote distribution</font></b></p>
        <br>
        <?php
        $stmt = $conn->query("SELECT p_division,no_of_registered_voters,no_of_votes FROM v_polling_division_pvn");
        $query = "SELECT p_division,no_of_registered_voters,no_of_votes FROM v_polling_division_pvn";
        ...
        foreach ($result as $voteData) {
            $pol_division[] = $voteData['p_division'];
        ...
        <table border="1" id="table" style="width:80%">
            <tr>
                <th style="width:15%">Polling Division</th>
            ...
            <script>
                ...
                function drawBarBasic() {
                    ...
                    var data = new google.visualization.arrayToDataTable([
                        ['Polling Division', 'Number of Registered Users', {role: 'annotation'}, 'Votes', {role: 'style'}, {role: 'annotation'}],
<?php
for ($i = 0; $i < $resultCount; $i++) {
    ?>[<?php echo "'" . $pol_division[$i] . "'," . $voter_count[$i] . ",'" . $voter_count[$i] . "' ," . $vote_count[$i] . ", '" . $color[$i] . "' , '" . $vote_count[$i] . "'" ?>],
<?php }
?>
                    ]);
...
if (isset($_POST['submit'])) {
    ...
    $url = $_POST["hidden_v"];
    $sql = "UPDATE image_url SET url='$url' WHERE id=4 ";

    if (mysqli_query($GLOBALS['conn'], $sql)) {
        echo "<script> window.location.assign('e_data_2021_pol_division_report.php'); </script>";
    }
}