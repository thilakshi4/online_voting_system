...
<main>
    <div class="container text-center style_form col-sm-10">
        <h2 class="py-4 bg-dark text-white rounded"><i class="fas fa-users-cog">&nbsp;</i>Manage Users</h2>
            ...
            <form action="" method="POST" class="w-50" onsubmit="return checkForm(this);">
                ...
                <div class="pt-2">
                    <div class="input-group">
                        <div class="input-group-text bg-warning">User Level &nbsp;<i class='fas fa-id-badge'></i>
                        </div><select class="form-select" id="user_level" name="user_level" onchange="change_subject()">
                            <option selected>--------Please Select-------</option>
                            <option value="1">Election Commission Officer</option>
                            <option value="2">District Officer</option>
                            <option value="3">Grama Niladhari</option>
                        </select>
                    </div>
                </div>
                <div style='color:red'> <label id="err_user_level"></label> </div>
                <br>

                <!-- jQuery UI auto complete-->
                <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/jqueryui/1.12.1/jquery-ui.css" />
                <script src="https://code.jquery.com/ui/1.12.1/jquery-ui.min.js"></script>
                <div class="pt-2" id="districts" style="display: none">
                    <?php inputElementManageUser_id("District", "<i class='fas fa-user'></i>", "District", "district", "", "district"); ?>
                </div>

                <div class="pt-2"  id="gn_divisions"  style="display: none">
        ...
        <div >
            <table class="table table-striped text-white table-dark" id="table" style="width:100%; ">
                ...
                <tr>
                            ...
                            <td style="color: white"><?php echo $row['user_level']; ?></td>
                            <td style="color: white"><?php echo $row['username']; ?></td>
                            <td hidden=""><?php echo $row['password']; ?></td>
                            <td style="color: white"><?php echo $row['district']; ?></td>
...
function getData() {
    if (isset($_POST['read'])) {
        ...
        $NIC = $_POST['search'];
        $sql = "SELECT * FROM staff_login WHERE NIC='$NIC'";
    } else {
        $sql = "SELECT * FROM staff_login ";
    }
...
function insertData() {
    ...
    $sql_e = "SELECT districtNo FROM e_district WHERE district='$district' ";
    ...
    $sql = "INSERT INTO staff_login(staff_id, username, password, user_level,name_with_initials,NIC,mobile_no,status,district,district_no,gn_division,gn_division_no) "
            . "VALUES ('$userid','$username','$password','$userlevel','$name_with_initials','$nic','$mobile_number','Successfully Registered','$district','$e_division','$gn_division','$g_division')";
...
function updateData() {
    ...
    $sql = "UPDATE staff_login SET username='$username',password='$password',user_level='$userlevel',"
            . "name_with_initials='$name_with_initials',NIC='$nic',mobile_no='$mobile_number',"
            . "district='$district',district_no='$e_division',gn_division='$gn_division',gn_division_no='$g_division' "
...
for (var i = 1; i < table.rows.length; i++)
{
    table.rows[i].onclick = function ()
    {
        ...
        document.getElementById("user_level").value = this.cells[4].innerHTML;
        ...
        document.getElementById("gn_division").value = this.cells[8].innerHTML;


        var user_level = document.getElementById("user_level");
        var district = document.getElementById("districts");
        var gn_division = document.getElementById("gn_divisions");
        district.style.display = user_level.value == "2" ? "block" : "none";
        gn_division.style.display = user_level.value == "3" ? "block" : "none";
