...
$res = mysqli_query($conn, "SELECT * FROM req_reg_officer_gn WHERE ref_number='$ref'");
...
<nav class="navbar navbar-expand-sm navbar navbar-dark bg-primary">
    <div ><a class="navbar-brand" href="request_gn.php.">&nbsp;&nbsp;Back</a></div>
...
<div align="center" class="style_form ">
    <form action = "" method = "post">
        <table border="1" id="table" style="width:70%">
            <?php
            ...
            <input type="text" name="username" id="username" hidden="" value='<?php echo $row['NIC']; ?>'>
    ...
    function insertData() {
        ...
        $sql = "INSERT INTO staff_login (staff_id, username, password,user_level,name_with_initials,
            NIC,mobile_no,status,gn_division,gn_division_no)
            SELECT ref_number, NIC, password,user_level,name_with_initials,
            NIC,mobile_no,'Successfully Registered',work_place,gn_division_no
            FROM req_reg_officer_gn
            WHERE ref_number='$ref'";
        $result = mysqli_query($GLOBALS['conn'], $sql);


        //delete relavent result from req_reg_officer_gn table
        $sql_del = "DELETE FROM req_reg_officer_gn WHERE ref_number='$ref'";
        $result_del = mysqli_query($GLOBALS['conn'], $sql_del);
        if (($result) AND ($result_del)) {

            sendsms_registation_success($mobile, $username, $password);
    ...
    function rejectData() {
        ...
        $sql_mob = "select mobile_no FROM req_reg_officer_gn WHERE `ref_number`='$ref' ";
        ...
        $sql = "INSERT INTO rejected_officer_registration (user_level,ref_number,name_with_initials,NIC,mobile_no,password,work_place,gn_division_no,p_division,district_no,commnent)
            SELECT user_level,ref_number,name_with_initials,NIC,mobile_no,password,work_place,gn_division_no,p_division,district_no,'$comments'
            FROM req_reg_officer_gn
            WHERE ref_number='$ref'";
        $result = mysqli_query($GLOBALS['conn'], $sql);
        ...
        if (($result) AND ($result_del)) {
            ...
            echo ' <script>setTimeout(function() '
            . '{swal({title: "Inserted to Rejected List ",'
