<?php
$title = 'Reset Password for Voter';
    ...
    <div class="collapse navbar-collapse" id="collapsibleNavbar">
        <ul class="navbar-nav">
            <li class="nav-item active">
                <div ><a class="navbar-brand" href="Login_voter.php.">&nbsp;&nbsp;Back</a></div>
            </li>   
        </ul>
    </div>
</nav>
...
function reset_password() {
    ...
    $sql = "SELECT * FROM voter_registration WHERE BINARY username='$username' AND mobile_no='$mobile'";

    $result = mysqli_query($GLOBALS['conn'], $sql);

    if (mysqli_num_rows($result) > 0) {
        $row = mysqli_fetch_assoc($result);
        if ($row['username'] == $username && $row['mobile_no'] == $mobile) {
            ...
            $sql_update = "UPDATE voter_registration SET password='$password' WHERE username='$username' AND mobile_no='$mobile'";
            ...
            if (mysqli_query($GLOBALS['conn'], $sql_update)) {
                ...
                . 'type: "success"}, '
                . 'function() { window.location = "Login_voter.php";});}, 500);</script>';
            }
        }
    } else {
